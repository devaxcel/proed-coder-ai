...
export default function Sidebar({
  ...
  const linkClass = (href: string) => {
    ...
    return `flex items-center rounded-md pl-5 pr-3 py-2 text-sm transition border-l-2 ${
      active
  ...
  const linkStyle = (href: string): CSSProperties => {
    const active = href === "/" ? pathname === "/" : pathname.startsWith(href);
    return active ? { backgroundColor: "rgba(212, 175, 55, 0.15)", borderLeftColor: THEME.secondary } : {};
  };

  const content = (
    <div className="flex h-full flex-col" style={{ backgroundColor: THEME.primary }}>
      <div className="px-3 py-4 border-b" style={{ borderColor: "rgba(212, 175, 55, 0.25)" }}>
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-none px-3 py-4 space-y-1">
              ...
              {isOpen && (
                <div className="space-y-0.5 pb-1">
                  {visibleLinks.map((link) => (
                    <Link key={link.href} href={link.href} onClick={onClose} className={linkClass(link.href)} style={linkStyle(link.href)}>
                      {link.label}
                    </Link>
                  ))}
                </div>
        ...
        {isAdmin && (
          <div className="pt-3 mt-3 border-t" style={{ borderColor: "rgba(212, 175, 55, 0.25)" }}>
            <div className="px-3 pb-1 text-xs font-bold uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.55)" }}>Admin</div>
      ...
      {isOpen && (
        <>
          ...
          <div className="lg:hidden fixed left-0 top-0 bottom-0 w-64 max-w-[85vw] z-50 shadow-2xl">
            <div className="flex justify-end p-2" style={{ backgroundColor: THEME.primary }}>
